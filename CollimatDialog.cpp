...
LOOP:
	...
	if (mesure_success_n == 0)
	{
		...
		dataSumList.InsertColumn(3, "X0", LVCFMT_CENTER, 100);
		dataSumList.DeleteColumn(4);
		dataSumList.InsertColumn(4, "Y0", LVCFMT_CENTER, 100);
...
bool Init_collimation()
{
	...
	if (abs(Center_f.x - Center_0.x) > 200 || abs(Center_f.y - Center_0.y) > 200)
	{
		int dataX[3] = { 0,0,0 };
		...
		if (up_down_flag == FORWORD)
		{
			...
			/*	CString xs[3];
				...
				while (!next_ok)
				{
					...
					if (endtime - begintime > 10) {
						AfxMessageBox(_T("下位机无响应AXISx，FORWARD，将退出发送程序，发送失败！！010200"));
						return false;
					}
			...
			else
			{
				...
				while (!next_ok)
				{
					...
					endtime = t / CLOCKS_PER_SEC;
					if (endtime - begintime > 10) {
						AfxMessageBox(_T("下位机无响应，将退出发送程序，发送失败！！ 010201"));
						return false;
					}
		...
		else if (up_down_flag == NEGITIVE)
		{
			...
			else
			{
				...
				while (!next_ok)
				{
					...
					endtime = t / CLOCKS_PER_SEC;
					if (endtime - begintime > 10) {
						AfxMessageBox(_T("下位机无响应，将退出发送程序，发送失败！！010200"));
						return false;
					}
		...
		if (up_down_flag == FORWORD)
		{
			...
			if (distance > 0)
			{
				...
				while (!next_ok)
				{
					...
					if (endtime - begintime > 5) {
						AfxMessageBox(_T("下位机无响应，将退出发送程序，发送失败！！010301"));
						return false;
					}
			...
			else
			{
				...
				while (!next_ok)
				{
					...
					endtime = t / CLOCKS_PER_SEC;
					if (endtime - begintime > 5) {
						AfxMessageBox(_T("下位机无响应，将退出发送程序，发送失败！！010300"));
						return false;
					}
		...
		if (up_down_flag == FORWORD)
		{
			...
			if (distance > 0)
			{
				...
				while (!next_ok)
				{
					...
					endtime = t / CLOCKS_PER_SEC;
					if (endtime - begintime > 5) {
						AfxMessageBox(_T("下位机无响应，将退出发送程序，发送失败！！010100"));
						return false;
					}
			...
			else
			{
				...
				while (!next_ok)
				{
					...
					endtime = t / CLOCKS_PER_SEC;
					if (endtime - begintime > 5) {
						AfxMessageBox(_T("下位机无响应，将退出发送程序，发送失败！！010101"));
						return false;
					}
		...
		if (up_down_flag == FORWORD)
		{
			...
			if (distance > 0)
			{
				...
				while (!next_ok)
				{
					...
					endtime = t / CLOCKS_PER_SEC;
					if (endtime - begintime > 5) {
						AfxMessageBox(_T("下位机无响应，将退出发送程序，发送失败！！010401"));
						return false;
					}
			...
			else
			{
				...
				while (!next_ok)
				{
					...
					endtime = t / CLOCKS_PER_SEC;
					if (endtime - begintime > 5) {
						AfxMessageBox(_T("下位机无响应，将退出发送程序，发送失败！！010400"));
						return false;
					}
...
//			AfxMessageBox(_T("将&Y角度向正向移动："));
			if (distance > 0)
			{
				...
				while (!next_ok)
				{
					...
					t = clock();
					endtime = t / CLOCKS_PER_SEC;
					if (endtime - begintime > 15) {
						AfxMessageBox(_T("下位机无响应，将退出发送程序，发送失败！！010500"));
						return false;
					}
			...
			else
			{
				...
				while (!next_ok)
				{
					...
					endtime = t / CLOCKS_PER_SEC;
					if (endtime - begintime > 15) {
						AfxMessageBox(_T("下位机无响应，将退出发送程序，发送失败！！010501"));
						return false;
					}
...
//求当所有数据计算出来的发散角的方差最小时distance的长度(有问题)
double GetDis(vector<double> &arr)
{
	const double pi = 3.141592654;
	const double AddL = 0.5;
	double Length = 10.0;
	int LengthIndex = 0;
	double arrSum = 0;
	double arrAve = 0;
	...
	while (Length < 18)
	{
		for (int i = 0; i < arr.size(); i++)
		{
			ang[i] = 2 * (atan(arr[i] * 5.5 / 1000 / (Length + AddL * i))) / pi * 180;	//根据采集到的数据求出对应的发散角
			arrSum += ang[i];
		}
		arrAve = arrSum / (arr.size());				//求发散角的均值
		//assert(arr.size() == ang.size());			//判断是否出错
		for (int i = 0; i < arr.size(); i++)
		{
			variance[LengthIndex] += (ang[i] - arrAve) * (ang[i] - arrAve);
		}
		variance[LengthIndex] /= arr.size();		//求发散角的方差
