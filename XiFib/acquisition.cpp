...
//**********************************************			
			...
			double dev=0;
			double Ldev=0;
			lps->ShowWin->m_Distance=Dist;
			Distance = lps->ShowWin->m_Distance;
/*******************************************************/
			//CEllipseFit LSellipse;
		...
		//	tempMat = tempMat - l;
			...
			if (m_Ellipse.xl!=0&&max>50)
			{
				...
				if(FiberModelFlag)
				{
					dataList.SetItemText(9,1, "—");//圆形光纤偏轴度
					/*lstr.Format("%.4lf",Distance);
					dataList.SetItemText(9,1,lstr);*/
				}
				else
				{
					lstr.Format("%.4lf",PartialAng);
					dataList.SetItemText(9,1,lstr);//楔形光纤偏轴度
				}
